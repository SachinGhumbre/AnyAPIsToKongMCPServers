# Traditional APIs to Kong MCP Servers

> **Powered by Kong Gateway & Kong Konnect**

An enterprise-grade, full-stack automation platform that converts traditional REST APIs into AI-native **Model Context Protocol (MCP) servers** on Kong Gateway — accepting either an **OpenAPI specification** or an **existing Kong proxy YAML** as input, and producing Kong configurations with `ai-mcp-proxy` plugins on every route.
...
sequenceDiagram
    ...
    Note over BE: deck openapi2kong → temp Kong YAML<br/>Add ai-mcp-proxy (conversion-only) per route<br/>Optional: Add /mcp-listener (listener mode)<br/>Tag: TraditionalAPIsToAIAPIs, mcp-tools
    ...
    BE->>DECK: deck gateway diff --select-tag TraditionalAPIsToAIAPIs
    DECK-->>BE: created/updated/deleted/unchanged counts
    BE-->>FE: Diff summary per file

    Dev->>FE: Deploy
    FE->>BE: POST /api/mcp-servers/deploy
    BE->>DECK: deck sync --select-tag TraditionalAPIsToAIAPIs
...
sequenceDiagram
    ...
    Note over BE: For EACH existing Route in Kong YAML:<br/>  → Add ai-mcp-proxy plugin (conversion-only)<br/>  → Tags: TraditionalAPIsToAIAPIs, mcp-tools<br/>Optional: Append /mcp-listener route (listener mode)
...
| **4. Deploy** | Sync config to Kong Konnect | `deck sync --select-tag TraditionalAPIsToAIAPIs` |

The `--select-tag TraditionalAPIsToAIAPIs` flag ensures `deck sync` only touches resources tagged by this platform — leaving all other existing Kong configurations untouched.
        ...
        └── For each path + method:
              1 Kong Route
                └── ai-mcp-proxy plugin (conversion-only)
                      ...
                      tags: [TraditionalAPIsToAIAPIs, mcp-tools, KONG_SERVICE_NAME_{svc}]
...
| `deploy_configs()` | Runs `deck sync --select-tag TraditionalAPIsToAIAPIs` |
...
Existing Kong YAML
  ...
  └── For EACH Route in yaml['routes'] (or service.routes):
        Add plugin:
          ...
          tags: [TraditionalAPIsToAIAPIs, mcp-tools]
...
| Tag-based Deployment | `--select-tag TraditionalAPIsToAIAPIs` |
...
| Tag | Purpose |
|-----|---------|
| `TraditionalAPIsToAIAPIs` | Scopes `deck sync` — only this platform's configs are touched |
...
deck gateway diff \
  ...
  --konnect-token kpat_... \
  --select-tag TraditionalAPIsToAIAPIs
